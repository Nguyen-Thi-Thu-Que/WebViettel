...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 text-xs font-semibold relative overflow-hidden bg-[#F8FAFC]">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#EE0033]/5 rounded-full filter blur-3xl opacity-40 pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-rose-300/10 rounded-full filter blur-3xl opacity-30 pointer-events-none" />
      ...
      <main className="w-full max-w-[480px] bg-white rounded-[24px] shadow-[0_20px_60px_rgba(0,0,0,0.08)] p-6 sm:p-10 transition-shadow duration-300 relative z-10 space-y-8 animate-fade-in-up">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-[15px] sm:text-[16px] font-medium text-slate-500 leading-normal">
            Đăng nhập để sử dụng hệ thống ViettelAI.
          </h2>
        </div>

        {/* Error Notification Alert Banner */}
        {errorMsg && (
          <div role="alert" className="flex items-center space-x-2 text-red-700 bg-red-50 border border-red-150 p-4 rounded-2xl text-xs font-semibold animate-scale-up">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex flex-col space-y-2 text-left">
            <label htmlFor="login-phone" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest pl-1">
          ...
          <div className="flex flex-col space-y-2 text-left">
            <div className="flex justify-between items-center px-1">
              <label htmlFor="login-password" className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                Mật khẩu tài khoản
              </label>
              <Link to="/forgot-password" className="text-[11px] text-[#EE0033] hover:text-[#D40032] hover:underline transition-colors font-bold">
                Quên mật khẩu?
              </Link>
            </div>
            <div className="relative flex items-center">
              ...
              <button
                ...
                className="absolute right-4 p-1 rounded-full text-slate-400 hover:text-slate-600 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500/20"
              >
